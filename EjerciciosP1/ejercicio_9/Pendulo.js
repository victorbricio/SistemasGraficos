...
class Cilindro extends THREE.Object3D {
 constructor(gui,titleGui, gui2,titleGui2, gui3,titleGui3) {
   super();
   ...
   this.createGUIA(gui3,titleGui3);
   ...
   //Variables booleanas de la animación de los péndulos

   this.boolPenduloGrande = true;
   this.boolPenduloPequeño = true;
 ...
 createGUIA (gui,titleGui) {
   // Controles para el tamaño, la orientación y la posición de la caja
   this.guiControlsA = new function () {
     this.penduloGrande = 0.0;
     this.velocidadGrande = 0.0;
     this.penduloPequenyo = 0.0;
     this.velocidadPequenyo = 0.0;
   ...
   // Las tres cifras indican un valor mínimo, un máximo y el incremento
   // El método   listen()   permite que si se cambia el valor de la variable en código, el deslizador de la interfaz se actualice
   folder.add (this.guiControlsA, 'penduloGrande', 0.0, 1.0, 1.0).name ('Péndulo Grande: ').listen();
   folder.add (this.guiControlsA, 'velocidadGrande', 0.0, 2.0, 0.01).name ('Velocidad: ').listen();
   folder.add (this.guiControlsA, 'penduloPequenyo', 0.0, 1.0, 1.0).name ('Péndulo Grande: ').listen();
   folder.add (this.guiControlsA, 'velocidadPequenyo', 0.0, 2.0, 0.01).name ('Velocidad: ').listen();

   folder.add (this.guiControlsA, 'reset').name ('[ Reset ]');
 }
 ...
 update () {
   ...
   if (this.guiControlsA.penduloGrande == 0){
      this.pivoteG.rotation.z = this.guiControlsG.giroG;
    }
   ...
   if (this.guiControlsA.penduloPequenyo == 0){
     this.pivoteP.rotation.z = this.guiControlsP.giroP;
   }
   ...
   //
   /// Animación del Péndulo Grande
   //

   if (this.guiControlsA.penduloGrande == 1){
     if (this.boolPenduloGrande){
      this.pivoteG.rotation.z += 0.01 * this.guiControlsA.velocidadGrande;
    }
    ...
    if (this.pivoteG.rotation.z >= 0.75){
      this.boolPenduloGrande = false;
    }
   ...
   //
   /// Animación del Péndulo Pequeño
   //

   if (this.guiControlsA.penduloPequenyo == 1){
     if (this.boolPenduloPequenyo){
      this.pivoteP.rotation.z += 0.01 * this.guiControlsA.velocidadPequenyo;
    }
    ...
    if (this.pivoteP.rotation.z >= 0.75){
      this.boolPenduloPequenyo = false;
    }

    if (this.pivoteP.rotation.z <= -0.75){
      this.boolPenduloPequenyo = true;
    }
   }
